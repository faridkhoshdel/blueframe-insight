...
export default function InvoiceVerifyPage() {
  ...
  const handleVerify = async (approved: boolean) => {
    try {
      ...
      setInvoice({ ...invoice, status: approved ? 'VERIFIED' : 'CANCELLED', verifiedAt: new Date() });
    } catch (e) {
    ...
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      <Card>
        ...
        <Descriptions bordered column={2}>
          ...
          <Descriptions.Item label="جمع کل">{new Intl.NumberFormat('fa-IR').format(invoice.subtotal || 0)} ریال</Descriptions.Item>
          <Descriptions.Item label="مالیات">{new Intl.NumberFormat('fa-IR').format(invoice.tax || 0)} ریال</Descriptions.Item>
          <Descriptions.Item label="تخفیف">{new Intl.NumberFormat('fa-IR').format(invoice.discount || 0)} ریال</Descriptions.Item>
          <Descriptions.Item label="مبلغ نهایی">
            <Text strong style={{ color: '#1890ff', fontSize: 16 }}>
              {new Intl.NumberFormat('fa-IR').format(invoice.total || 0)} ریال
            </Text>
          </Descriptions.Item>
        </Descriptions>
        ...
        <Table
          ...
          columns={[
            ...
            { title: 'قیمت واحد', dataIndex: 'unitPrice', render: (p: number) => new Intl.NumberFormat('fa-IR').format(p || 0) },
            { title: 'جمع', dataIndex: 'total', render: (t: number) => <b>{new Intl.NumberFormat('fa-IR').format(t || 0)}</b> },
          ]}
        />
